...
Final rendered short ready to download
...
Music analysis with energy lanes and markers
...
Target length and slot selection
...
Storyboard clip slots and composed preview
...
Hook title and emphasis styling
...
Caption configuration with auto-transcribeCaption preview on composed video
...
SFX section with beat-aligned cues
...
Open [http://localhost:5173](http://localhost:5173) (Vite proxies `/api` to the API on port 8765).
...
Open [http://127.0.0.1:8765](http://127.0.0.1:8765) — serves the built UI from `web/dist/`.
...
Open [http://127.0.0.1:8765](http://127.0.0.1:8765). Bind-mounts:
